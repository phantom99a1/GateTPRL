...
{
    public class MessageHelperReposFirmReport : MessageHelper
    {
        private MessageReposFirmReport c_message;

        #region #Tag const

        private const int TAG_ClOrdID = 11;
        private const int TAG_QuoteID = 117;
        ...
        private const int TAG_SettDate = 64;
        ...
        public MessageHelperReposFirmReport(FIXMessageBase message)
            : base(message)
        {
            c_message = (MessageReposFirmReport)message;
        }
        ...
        public override void BuildBody(StringBuilder sb)
        {
            ...
            sb.Append(TAG_SettDate.ToString()).Append(Common.DELIMIT).Append(c_message.SettlDate).Append(Common.SOH);
        ...
        public override bool ParseField(Field field)
        {
            ...
            {
                bool _isSuccess = false;
                c_message.MatchReportType = Utils.Convert.ParseLong(field.Value, ref _isSuccess);
                if (_isSuccess == false) return false;
            }
            //
